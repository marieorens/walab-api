@php
    $user = Auth::user();
    $isLabo = $user && $user->role && $user->role->label === 'laboratoire';
@endphp

@extends($isLabo ? 'laboratoire.layout' : 'layout')
...
<script>
    ...
    async function loadNotifications() {
        try {
            ...
            console.log('Number of notifications:', data.data ? data.data.length : 0);
            ...
            if (data.success) {
                ...
                document.getElementById('no-notifications').innerHTML = '<div class="alert alert-warning">Aucune notification pour le moment</div>';
                document.getElementById('no-notifications').style.display = 'block';
            }
        } catch (error) {
            console.error('Erreur chargement notifications:', error);
            document.getElementById('no-notifications').innerHTML = '<div class="alert alert-danger">Erreur de chargement: ' + error.message + '</div>';
            document.getElementById('no-notifications').style.display = 'block';
        } finally {
