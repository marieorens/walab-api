...
class GenerateWeeklyWithdrawals extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'wallets:generate-weekly-withdrawals 
                            {--periode= : La période au format Y-Wxx (ex: 2025-W51). Par défaut: semaine précédente}
     ...
     * @var string
     */
    protected $description = 'Génère les retraits hebdomadaires pour tous les agents ayant un solde positif';
    ...
    {
        $periode = $this->option('periode') ?? now()->subWeek()->format('o-\WW');
        
        $this->info("🔄 Génération des retraits hebdomadaires (Agents) pour la période: {$periode}");
        $this->newLine();

        try {
            $results = $this->walletService->generateWeeklyWithdrawals($periode);

            $this->info("✅ Retraits hebdomadaires générés avec succès!");
            $this->table(
                ['Métrique', 'Valeur'],
                [
